...
- **Translation Keys** - every string lives in a single [react-i18next](https://react.i18next.com/) catalogue (`src/i18n/locales/en.json`) rather than being hard-coded across components, organised into namespaced keys (`filters`, `kpi`, `table`, `charts`, `chat`, `theme`, …) with interpolation for dynamic values (`{{matched}} matched filters`). This keeps copy consistent and easy to change in one place (and leaves the door open to localisation later).
